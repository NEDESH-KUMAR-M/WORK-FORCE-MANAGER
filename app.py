    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                if action == 'start':
                    sheet5.update_cell(task_id, 4, current_time)  # Column D (Start Time)
                    sheet5.update_cell(task_id, 9, 'In Progress')  # Column I (Status)
                    ...
                    if start_time:
                        try:
                            ...
                            sheet5.update_cell(task_id, 5, current_time)  # Column E (End Time)
                            sheet5.update_cell(task_id, 8, hours_str)     # Column H (Hours)
                            sheet5.update_cell(task_id, 9, 'Completed')   # Column I (Status)
                            flash('Task completed successfully!', 'success')
                        except ValueError as e:
                            flash('Error calculating time duration', 'error')
                            print(f"Time calculation error: {e}")
                    else:
                        flash('Cannot complete - task was never started', 'error')
                else:
                    flash('Invalid action specified', 'error')

                return redirect(url_for('time_emp'))

            except Exception as e:
                flash('Error processing your request', 'error')
                print(f"Error in time_emp POST: {e}")
                return redirect(url_for('time_emp'))

        # Handle GET requests (or fallthrough for POST if needed)
        try:
            # Skip first 2 formatting rows
            all_values = sheet5.get_all_values()[2:]
            records = []

            for i, row in enumerate(all_values, start=4):  # Row 4 is first data row
            ...
            print(f"Error in time_emp GET: {e}")
            return render_template('time_emp.html',
                                not_started=[],
                                in_progress=[],
                                completed=[])
        ...
        print(f"Unexpected error in time_emp: {e}")
        return redirect(url_for('empdashboard'))
...
def payslipad():
    try:
        ...
        if len(all_data) > 1:  # Check if there's data beyond headers
            headers = all_data[0] if all_data else PAYSLIP_HEADERS
            for idx, row in enumerate(all_data[1:], start=2):
                ...
                record = dict(zip(headers, row))
                record['row_id'] = idx  # Add the row number for reference
                
                status = record.get('Status', '').strip().lower()
                if status == 'pending':
                    pending_requests.append(record)
                elif status == 'fulfilled':
                    fulfilled_requests.append(record)
        ...
        flash('Error loading payslip data', 'error')
        return render_template('payslip_admin.html', pending_requests=[], fulfilled_requests=[])
    ...
    try:
        ...
        try:
            ...
            drive_service = build('drive', 'v3', 
                                credentials=creds,
                                static_discovery=False,  # Important for some environments
                                cache_discovery=False)  # Prevents discovery doc issues

            # Verify connection
            try:
                drive_service.files().list(pageSize=1).execute()
            except Exception as test_error:
                print(f"API Connection Test Failed: {test_error}")
                flash('Failed to connect to Google Drive API', 'error')
                return redirect(url_for('payslipad'))
            ...
            update_data = [
                ('Status', 'Fulfilled'),
                ('File Name', filename),
                ('Download Link', uploaded_file.get('webViewLink')),
                ('Upload Date', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            ]
            
            for header, value in update_data:
                if header in headers:
                    col = headers.index(header) + 1
                    payslip_sheet.update_cell(int(row_id), col, value)
            
            flash('Payslip uploaded successfully!', 'success')
            return redirect(url_for('payslipad'))
            
        except Exception as drive_error:
            print(f"Drive API Error: {str(drive_error)}")
            
            if "Unable to find the server" in str(drive_error):
                flash('Cannot connect to Google servers. Check your internet connection or firewall settings.', 'error')
            else:
                flash(f'Google Drive error: {str(drive_error)}', 'error')
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        print(f"Failed to clean temp file: {clean_error}")
                    time.sleep(0.5)
